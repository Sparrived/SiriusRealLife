...
⚠️ **对你的意义**：你的三层里 `longterm` 是"压缩摘要"，但没有**什么时候压、压什么、怎么压**。上面这套是现成的，而且它解释了为什么需要：论文的例子是——只给原始观察时，问 Klaus"你最想和谁待一小时"，他选了**见面最频繁但并不亲近**的宿舍邻居 Wolfgang；有了反思层，他选了**真正有共同兴趣**的 Maria。
...
| # | 现状 | 建议 | 依据 |
|---|---|---|---|
| 1 | 意识流 `working` = 最近 N 条 | 加**检索函数**：`recency(0.995 衰减) + importance(LLM 1–10 打分)`，先不做 relevance（省掉 embedding 依赖） | §1.2 |
| 2 | `longterm` = "压缩摘要"，无触发条件 | 用**重要度累加阈值**触发反思；两段式（先问"3 个值得回答的问题"→ 再问"5 条洞见"），洞见带来源指针 | §1.3 |
| 3 | 状态 `tools` 是硬白名单 | 改成**建议 + 例外硬封锁**（phoebe 教训） | §3.3 |
| 4 | 无计划层，纯随机/加权分派 | v1 至少加**一个每日粗粒度意图**，分派在其约束下进行 | §1.4 |
| 5 | "状态触发事件"未定义裁决者 | 明确引入 **GM/裁决器**角色：状态只说"想做什么"，GM 说"结果如何" | §2 |

### 6.2 建议写进验收标准

来自论文自报的失败模式（§1.5）——这三条可以直接当"跑 30 tick 之后人工看一遍"的检查项：

1. 有没有**取不到相关记忆**（该想起的没想起）
2. 有没有**编造没发生过的事**
3. 有没有**说话越来越像客服**（继承 LLM 的正式腔）

另加 ai-town 的一条：**能暂停、能存档、能回放**（§3.2、§4）。

### 6.3 明确不改

- **不做 embedding / 向量库**：v1 用 recency + importance 顶住。等 AMKR 侧确认有 embedding 端点，或人格明显失忆，再加 relevance。
- **不引入 Concordia / LangChain 之类框架**：架构思路照抄，代码自己写（符合 AGENTS.md 反目标）。
- **不自建计划器的递归分解**：那是为 25 个 agent 跑两天的研究项目准备的，我们 1 个 agent 用不上。

### 6.4 你这套设计里**没有现成答案**的部分
...
2. **心境与状态的显式正交**。论文用 plan（时间尺度一致性）+ importance（情绪权重）间接实现了类似效果，但没有把"心境"作为独立连续量抽出来。你的 R2 在这点上是**比论文更干净的抽象**——⚠️ 但也意味着**没有经验可抄，要自己调**。
3. **事件抢占状态**。Generative Agents 有 re-planning 但没有"抢占"语义；Concordia 的 GM 有裁决但没有状态抢占。这一块要自己设计，建议**第一次就记好 R6 日志**，否则抢占逻辑出问题会极难查。
