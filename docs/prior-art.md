...
⚠️ **对你的意义**：这正是"**没有计划层的逐次决策**"最该被质疑的地方——只看这一步合不合理，不保证整条轨迹像个人。论文的解法是**计划层 + 随时重规划（re-planning）**。

⚠️ **我的建议（本轮已部分落地）**：`for_ticks` 让模型主动声明"下次什么时候再问我"，这已经是一种**最粗粒度的计划**：模型可以在 `why` 里说"我想把这件事做完"，把检查点放到 20 tick 之后，而不是每 3 tick 就被问一次、每次都重新掷一次方向。这拦住了"同一件事被拆成碎步反复重启"，但**仍不等于当日意图**——它没有跨状态的连贯目标。

**待办**：v1 若要补，最省的是**一个粗粒度的当日意图**（哪怕就是"今天想干什么"一句话，每天生成一次），把它放进常驻 prompt。成本极低，却能拦住"吃三次午饭"这类问题。见 §6.3。
...
⚠️ **两点观察**：
- 它把转移决策**交给 LLM**，我们**也已交给 LLM**（R3 修订）——不再有权重抽取。代价就是这里说的"每次转移都要一次调用（贵、慢、不稳定）"。我们的缓解手段是：**把决定时机交给模型自己定**（`for_ticks`），而不是框架每隔几 tick 强行问一次；以及**失败一律降级为 `stay`**（绝不随机兜底），因此"不稳定"表现为"原地不动一会"，而不是"人格随机跳变"。
...
⚠️ 这直接印证了 AGENTS.md 里的 **R6（每次决定都留结构化日志）** 和 **R8（只认 tick 序号）**——可回放/可分叉是这类系统的**一等需求**，不是调试附属品。

我们现在的 R6 记 `from/to/reason/why/for_ticks/完整候选清单`，**没有随机数可记**（框架里没有随机）——可复现性来自"同一个模型给同样的输入必然给同样的决定"加 R8 的确定性 tick。因此**回放**仍然免费，但**分叉**的性质变了：分叉不再是"换个种子跑出另一条轨迹"，而是"同一条轨迹上换一个决定，看后面怎么走"。

⚠️ **一处已知弱点**：日志记的是模型**给了什么**理由，不是它**为什么**给。要真正复盘"她为什么从没去过睡觉"，靠的是候选清单里的 `Blocked` 原因（"此刻不满足进入条件"）——那能区分"框架没给她这个选项"与"她看到了但没选"。这正是 `Blocked` 必须落盘、而不能只写进 prompt 的原因。
...
| 4 | 逐次决策无法保证**时间尺度一致** | 已用 `for_ticks` 部分缓解（模型自己声明下次何时被问）；当日意图仍不做，风险见 §6.3 |
...
- **不自建计划层 / 每日意图**：MVP 收敛掉。⚠️ **已知风险**：逐次决策可能产生"12 点吃午饭、12:30 又吃、1 点再吃"这类时间尺度不一致的行为（§1.4）。`for_ticks` 只缓解了一部分（模型能说"我要把这件事做完，20 tick 后再问我"），但没有跨状态的连贯目标。若 Phase 1 验收时观察到，补一个每日粗粒度意图即可。
...
1. **自主性 + 自己决定下一秒干什么**。所有 FSM×LLM 项目（`fsm-llm`、`phoebe`、`milkie`）都是**外部输入驱动**的：用户说话、告警触发。**"没人理的时候自己决定下一秒干什么"几乎没有现成参考**，只有 Generative Agents 的 plan/reflect 循环沾边。
...
4. **"状态是一段订阅"**。所有参考项目里，状态要么是流程节点（走完就过），要么是外部事件驱动的响应。**没有谁把"持续订阅某个信息来源"做成状态的核心语义**——而这正是"她在刷手机时，QQ 消息即时呈现在眼前；她没在看时，消息只是静默堆积"的实现方式。Generative Agents 有 re-planning 但没有这种订阅语义。

   > 早期设计里这块是"事件抢占状态"。**已经推翻**：抢占让"谁在什么时候打断她"变成框架的硬编码判断，而真人不会因为手机响一声就放下手里的事。改为订阅 + 多个决策入口注入理由（R11/R12）后，同样的场景由她自己权衡。要自己设计的地方从"抢占优先级"变成了"**理由怎么呈现给她**"。
