...
MVP 的 4 个状态（是否订阅 QQ 标在第二列）：

| 状态 | 订阅 QQ | 说明 |
|---|---|---|
| `scrolling_phone` | ✅ **看 QQ 就发生在这里** | 进入时返回最近 N 条，驻留期间新消息持续泵入 |
| `idle` | ❌ | 发呆，消息静默入队 |
| `working` | ❌ | 工作，消息静默入队 |
| `sleeping` | ❌ | 睡着时手机响了也不看（信息门控，不是抢占规则） |

状态表里**没有权重**：换不换、换去哪、待多久都由 LLM 通过 `stay` / `enter_state` 决定（R3）。
...
**记忆部分按 [`memory.md`](memory.md) §9 Phase 1 做**：unread 队列、三类消息标记（`@` 只是更显眼，不打断）、已读游标、关键词打捞返回整段、记忆曲线 + Shadow、升格（源条目删除）。
...
1. 不自锁、不死循环
2. 每次状态决定都有结构化日志（R6）——含模型给的理由与当时的完整候选清单
3. LLM 调用期间状态机**不被阻塞**（R4）—— 调用中仍能收事件、仍能继续泵入订阅的消息
4. 同一状态不连续进入两次（R3）—— 当前状态不进候选清单
5. 不在 `scrolling_phone` 时 QQ 消息**不进 prompt**；`@` **不打断状态**，但未读与烦躁随之增长；`sleeping` 不订阅 QQ，内容对它不可见
6. 有记忆因长期不打捞而沉入 Shadow，且 Shadow 内容**不出现在 prompt**

以上 6 条已由 [`internal/acceptance`](../internal/acceptance/acceptance_test.go) 逐条覆盖（装配方式与 `cmd/sirius` 一致）。另外补了几条原来没写进验收、但会悄悄坏掉的：

- **决定可复现**：同一个假 LLM 跑两遍必须产生同一串状态，且每步都能在日志里查到当时的理由与候选清单
...
- **精力收支平衡**：睡一觉真的补得回来，且算术前提本身被单独断言（`TestSleepRecoversFasterThanDecay`）

> 后几条的由来值得记住：前端、单测、日志全都正常，界面上意识流也一直在滚动——
> 但整个系统其实只是一个状态机加四条硬编码旁白。**"看起来在动"不等于"链路是通的"。**
>
> 同理，改"状态由 LLM 决定"之后，一个只回文本、不会调工具的假 LLM 会让 agent 永远停在初始状态，而所有断言照样通过。**因此验收用的假 LLM 必须真的调 `enter_state`**，否则测的只是降级路径。
...
| 回复工具 `send_message` | **已落地（无出站通道）** | 只在工具结果轮提供（**不带状态工具**，回复不改状态）。她的话进意识流（动作）+ 进待选区（同一段翻阅，importance 6）。`Outbox` 缝口已就位，但**还没接真实发送端**：`Outbox` 为 nil 时话只进记忆并留一条 `outbox_missing` 日志，不假装成功。快接上时只需实现 `fsm.Outbox` |
| Sirius 自身鉴权 | **已落地** | 登录页 + 会话 cookie（`/login`），同时保留 HTTP Basic 供 curl/脚本使用；包住全部路由**含 `/amkr/`**，只有 `/login` 与 `/api/v1/health` 例外。**cookie 是单点进 `/amkr/` 的前提**：浏览器导航带不上 `Authorization` 头，Basic 做不到这件事。它的存在解除了 AGENTS.md §4 的前置条件——此前只能绑回环就是因为 `/amkr/` 等同于 AMKR 的完整管理权限（能读到上游 key）。当前入口：Cloudflare Tunnel `sirius.sparrived.xyz` → 宿主回环。**仍然不要用 `--allow-ops`**：那会放行 AMKR 的宿主机运维接口 |
